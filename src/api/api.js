  ...
  constructor(config = {}) {
    ...
    this.axiosInstance.interceptors.response.use(
      ...
      error => {
        ...
        if (status === 401 && error.config?.baseURL === BASE_URL) {
          console.error('[API] 401 Unauthorized - Token invalid or expired');
          // Instead of forcing immediate logout which clears all state,
          // mark session as expired and redirect user to login page so
          // they can re-authenticate. Keep token in store for debugging.
          try {
            getStore().dispatch({ type: 'SESSION_EXPIRED' });
          } catch (e) {
            console.error('[API] Failed to dispatch SESSION_EXPIRED', e);
          }

          // On Android handle plugins cleanup
          if (isAndroid()) {
          ...
          // Redirect to login/signup but don't immediately clear persisted store
