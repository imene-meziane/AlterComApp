...
// Allow explicit override via REACT_APP_API_URL (useful in dev)
const EXPLICIT_API_URL = process.env.REACT_APP_API_URL || null;
...
// If an explicit API URL env var is set, prefer it; otherwise prefer DEV_API_URL
// If running on localhost, default to http://localhost:4000/ for local backend
const LOCALHOST_FALLBACK = 'http://localhost:4000/';

const RAW_API_URL = isCordova()
  ? 'https://api.app.cboard.io/'
  : EXPLICIT_API_URL || DEV_API_URL || (host.includes('localhost') ? LOCALHOST_FALLBACK : `${window.location.protocol}//api.${host}`);
